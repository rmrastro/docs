...
%\author{Renbin Yan (Survey Scientist) \\ Composition Strategic Committee ~~ Kinematics Strategic Committee \\ MaNGA Team}
...
%This is just for testing citations\citep{weijmans09, sales11}.

\noindent{\bf MaNGA's Key Science Questions:}
\begin {enumerate} 
...
%\item Is star formation in present-day galaxies quenched by energetic
%  input from accreting black holes, from star formation, or from some
%  combination of the two? {\em (Section \ref{sec:winds})}

%\item How is the shutdown of star formation regulated by
%  externally-driven processes in massive groups and clusters? {\em
%    (Section \ref{sec:high-density})}
...
% \item Does galaxy growth at low and high redshifts proceed in the same
%   way? {\em (Section \ref{sec:high-z})} {\bf what happened to this section?}
\end {enumerate}
...
will greatly leverage the legacy value of the MaNGA data products. %At
%this time, we have only just begun to formulate the observational plan
%to acquire the stellar library. If indeed this can be accomplished
%during bright time, we are confident our hardware design is adequate
%for the task.
...
%% ----------------------------------------------------------------------------------------------------
%%   MaNGA Key Science Requirements
%% ----------------------------------------------------------------------------------------------------
\newpage
...
%Finally, the strength of metallicity gradients in disk galaxies will
%provide important constraints on recent gas accretion processes.
% 
...
% This section could use some references
...
mechanisms that shape the outer disk. %By stacking samples of several
%hundred edge-on galaxies, a chemical decomposition of thick and thin
%disks may be possible for the first time outside the Local
%Group. 
...
\item[P1.2] Measure the radial gradients in gas phase metallicity ($\log (O/H)$) to 0.04 dex per \Reff\ using the ``strong line'' methods \citep{kewley02,
  ...
  [S~III]$\lambda\lambda 9069,9532$. This requires measuring the metallicity per binned fiber annulus to better than 0.05 dex.  %This requirement is driven by the goal of
%  measuring metallicity gradients to 0.04 dex per \Reff. 
  %\citep[$\sim0.04$~dex per kpc][]{vanZee1996}. 
...
\item[P1.3] Nebular metallicity measurements in three or more equally
  ...
  gradients due to annular binning to better than $\sim30$\%. %For a typical gradient of 0.1~dex/\Reff, this would give
  %an uncertainty of 0.03~dex/\Reff, better than the required 0.04~dex/\Reff.
  %This will require spatial sampling by a minimum
%  of 3 fibers in the radial direction. 
...
%  from 
%  better than $\pm0.05$ dex per radial bin.  This requirement is motivated
%  by simulations \citep[e.g.,][]{lackner12} which suggest that
%  accreted stars are older and more metal-poor than stars formed in-situ 
%  by approximately 0.15 dex.  % Actully these simulations were for 
  % early type galaxies
   
%\item[P1.6] Measurements of gas and star velocities in individual
%  fibers %with errors of less than 25~km~s$^{-1}$ 
%  to enable identification of counter-rotating components in galaxies
...
%, and to probe an under-explored regime in galaxies.

%  This enables measurements of accretion processes (gas and
%  stars/satellites) and their effect on the stellar population
%  gradients. 
%The choice of 1.5~$R_e$ is a compromise between what is
%  interesting and what is realistic for stellar population studies 
%  given the steeply declining surface brightness profile.
...
% Moran finds most of the action at R>R90 = 2.85 R_e 
...
% Some statistics from Moran 2012
...
%derived from analysis of absorption indices \citep[e.g.][]{johansson12} 
...
%\item[P2.3] Measurements of gas and star velocities and velocity
%  dispersions in individual fibers with errors of less than
%  25~km~s$^{-1}$ to enable kinematic merger identification and the
%  study of non-circular motions and gas flows in disks.

%\item[P2.3] Measurements of the Toomre Q-parameter to better than
%  \red{$\pm$ XX } to test whether bars are associated with unstable
%  disks.  To achieve this requires measurements of the stellar velocity
%  dispersion in individual fibers to better than \red{XX\%} and
%  measurement of the local surface density to \red{ XX\%}.
...
%To estimate the emission line S/N in the outer parts of 
%galaxies, we use the SDSS-I spectra of potential targets and make 
%the conservative assumption that the EWs are constant
%with radius.  (In practice, they are likey increase with radius.)  

%In most (but not all) science cases it is wise to limit
%analysis to galaxies or parts of galaxies where the H$\beta$ EW is
%greater than 2.5~\AA.  H$\beta$ is crucial for diagnostics of dust
%attenuation, star formation, and metallicity, but it suffers from
%strong underlying stellar absorption.  This will be corrected for by
%fitting and removing the stellar continuum before measuring the
%nebular lines, but this can be a significant source of uncertainty in
%low EW lines.  In the following we therefore assume EW(H$\beta$)
%$>2.5$~\AA\, which roughly corresponds to EW(H$\alpha$) $> 10$~\AA.
%Since the spectrograph has a dispersion of 69 km~s$^{-1}$~pix$^{-1}$
%and we will be spatially resolving galaxy rotation curves, we assume
%that most of the nebular flux will be concentrated in a single pixel.
%Thus, the flux in a pixel centered on emission line will be $f_{line}
%= f_{cont}*EW$ and the S/N of the line will be $S/N_{line} = \sqrt EW
%\times S/N_{cont}$.  To achieve a given S/N on a H$\beta$ line with
%EW = 2.5~\AA\ therefore requires $S/N_{cont} = S/N_{line}/\sqrt EW =
%0.63~S/N_{line}$.

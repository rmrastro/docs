...
\vspace*{-1.5cm}
 ...
 % \textbf{\textsf{Mid-scale RI-1 (M1:DP): Training for Big Astronomical Data with Keck-FOBOS}}
...
\noindent {\bf Overview:} This Mid-scale RI-1 (M1:DP) \emph{Design} proposal...
...
% brightness in different broadband filters (projected into a
% two-dimensional space) and observed spectroscopic redshift (indicated
% by the color map).  SOMs guide the optimal construction of training
% samples by highlighting which galaxy classes require targeting.
% \emph{Right:} The spectra associated with localized SOM regions have
% similar spectra, as well as similar redshifts. \textcolor{red}{JAN:
% Contra the previous text, I would say that the similarity of the
% spectra is neither remarkable nor surprising, since to be assigned to
% the same cell the galaxies have to have extremely similar SEDs, and
% hence spectra.  A bigger open question is can you get similar apparent
% SEDs from multiple redshifts (e.g., are there degeneracies with other
% parameters); the SOM is not necessarily one-to-one with redshift, but
% should be deterministic of spectral shape in any event.} }
...
%\begin{wrapfigure}{r}{\textwidth}\small
%%
%\includegraphics[width=\textwidth]{Hemmati18_Fig8_VVDS_spec.png}
%%
%\caption{{\it Left}: A Self-Organizing Map
%\citep[SOM;][]{1990Natur.346...24K} from \citet{hemmati18} encoding the
%relation between colors in an LSST+WFIRST-like color space and redshift.
%Position on the SOM is associated with a position in the
%multi-dimensional broad-band color space of galaxies.  The redshift,
...
%\citep[cf.][]{2011arXiv1110.3193L, 2015arXiv150303757S, 2017MNRAS.465.1454H, 2018ApJS..239...18A, 2018arXiv180901669T}, 
...
%Typical use broad photometric bands, which fundamentally limits the
%amount of information available on the spectral energy distribution of
%galaxies (SED) and therefore on the galaxies redshift. 

% Inaccurate redshift information is therefore one of the dominant
% sources of systematic uncertainty in these surveys \citep{hemmati18}
% that are quite sensitive to modelling biases
% \citep{2018MNRAS.476..151E, 10.1093/mnras/sty2902}.
...
High-Precision LSST Photometric Redshifts with Targeted Training Spectroscopy}}.
Delivering optimal photo-$z$s with minimal errors per object will
require sets with $>10,000$ spectra.  These data allow for training
purely machine-learning-based algorithms, appropriately expand our
sampling of galaxy spectra, and minimize calibration errors for
template-based and/or hybrid algorithms.  Specifically, we will build a
methodology for sample design that will obtain galaxy spectra directly
relevant to regions of parameter space that are most difficult for
current photo-$z$ training estimation, as described below.\footnote{
%
Of course, the design of this sample also benefits the data-science
challenges we detail in Section \ref{sec:galaxies}.}
%
Keck-FOBOS is ideally suited to observe these galaxy samples considering
the requirements in \citet{newman15}.  

%This will not only improve photometric redshift accuracy, but will also allow us to derive galaxy properties like stalla mass or age for these photometric samples. This effectively enables the study of stellar evoution on cosmological scales. }

A well-designed galaxy sample that efficiently populates critical
regions of galaxy color space and avoids debilitating selection biases
is difficult to define.  A state-of-the-art technique to represent the
color space in low dimensions is to construct a Self-Organizing Map
(SOM, Fig.~\ref {fig:SOM}), which provides a representation of a
high-dimensional input space in 2D grid cells. The grid cells shown in
Fig.~\ref{fig:SOM} represent coherent cells of input in high dimensions.
We propose to use simulated spectroscopic and photometric survey data
that mimic the expected selection biases of the FOBOS spectroscpopic
sample.  We will then benchmark the coverage of photometric color space
with respect to our spectroscopic sample and, similarly, the color-space
coverage of our derived galaxy models as a function of the optimal
observing strategy.  Obtaining these simulations and benchmarking the
color-space coverage is a time-consuming task, leading us to consider
Bayesian optimization \citep[see e.g.][]{2017arXiv170306240K,
2018arXiv180512168P, 2019arXiv190111515N}.

\end{enumerate}
% This methodology is especially well-suited to our sample design,
% particularly to isolate the minimal coverage of spectroscopic
% observations that optimize sampling in color space.  As the 

% that requite considerable time to evaluate, which is certainly the
% case here.  The idea of these methods is to guide the optimization
% steps via an interpolation scheme.  As this methodology is very
% general and can be used to jointly optimize multiple target functions,
% we will also be able to account for the requirements of different
% science goals, that might need slightly different observation
% strategies. 

% Our goal is to design and deliver an optimized set of spectroscopic
% redshifts which will enable photometric redshifts to be accurately
% 'painted on' to LSST imaging-only objects, and thereby improve both
% dark energy and galaxy evolution science from LSST.
...
\begin{wrapfigure}{r}{0.45\textwidth}\small
%
...
\item[] {\textsf {\large  Data-Science Challenge
...
\medskip
\chal{gaia} 
\item[] {\textsf {\large Data-Science Challenge \ref{gaia}:
...
%\includepdf[pages=-,landscape=true]{FOBOS_V05_export.pdf}

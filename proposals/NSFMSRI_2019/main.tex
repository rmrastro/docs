...
Yuan Sen:
I love the new version of the draft. Just a few minor comments:

- Probably you don't have any space left, but I feel like the connection with PLATO and TESS, and getting stellar ages through machine learning, connecting PLATO/TESS to FOBOS can be exciting.

- when introducing The Payne, it might be worth stressing that we are getting 16 elemental abundances from *low-resolution* spectra. Currently, from the text, it is unclear -- getting 20+ elements from high-resolution spectra are "trivial" :P

- data-challenge 5: we really have all the expectation of getting alpha/Fe from multiband photometry. And getting alpha/Fe is really the key. With [alpha/Fe]-[Fe/H], we can, in principle, reconstruct the star formation history of individual disrupted galaxy better, and hence reconstructing the halo history (dwarf galaxy mass function. essentially)

- data-challenge 6: although it is well accepted that M31 and MW has different accretion history (e.g. the number of globular clusters at M31 is much higher), I am not entirely sure if the chemistry is much different though.

- data-challenge 7: It seems a bit misleading to say we will get data-driven distance. It is more like we can get data-driven "absolute magnitude", from which we can derive the distances.

Just some minor comments. The draft is in GREAT shape!

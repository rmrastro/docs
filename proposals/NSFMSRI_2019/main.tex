...
(e.g., from LSST) is in-hand, while mock spectroscopy will be provided by a Keck-FOBOS instrument simulator, an initial
version of which has already been developed.  Further advances on the FOBOS simulator will be supported by this
proposal and include improved error modeling and simulating systematic effects from detector artifacts, image quality
aberrations informed by the emerging detailed optical design, and variable observing conditions.

The resulting success in addressing each Data Science Challenge will define a level of readiness and set requirements
on the associated Keck-FOBOS training sets required.  Preliminary observing design and a description of required
operational modes will begin with this proposal, with detailed survey and execution plans to be completed in the next
phase of this project (MSRI-2).  The anticipated survey will consist of multiple, combined programs with integrated
tiers of exposure time and target density.  

...
\textbf{\textsf{Mid-scale RI-1 (M1:DP) Keck-FOBOS:\\ Building Comprehensive, Data-Driven Models of a Universe in Transition}}
...
Milky Way Galaxy and its Local Group provide unparalleled opportunities
to study galaxies in exquisite detail, by virtue of being able to
spatially resolve their constituent parts.  In fact, the Gaia satellite
(ref) is currently revolutionizing our understanding of the Milky Way
Galaxy, providing distances and on-sky motions for more than a billion
stars spanning the full extent of its disk.  These data represent a
gigantic leap forward in our ability to construct a history of the Milky
Way by effectively tracing its stellar populations and dynamical
structures back in time.  However, the Gaia data are still limited:
...
only 0.1\% will have measured chemical abundances.  Existing and ongoing
surveys, like SEGUE, RAVE, and APOGEE, and planned surveys, like WEAVE
and 4MOST (refs), provide critical supplements, but these efforts only
just begin to fulfill the potential of the Gaia astrometric data.
Directly observing the all 1.7 billion stars is simply untenable for the
foreseeable future.
...
spectra where those parameters are unknown.  In their application, they
...
In both of these applications, there are critical regimes that 
...
reach for FOBOS, a well formulated training set that spans the

\medskip \noindent {\bf Data Science Challenge 6: Intelligent target
allocation schemes that allow for the integration of high target density
programs (such as discussed in Section X) with low density programs,
such as this one, to appropriately meet the needs of both surveys.} See
Section X.


- SDSS revolutionized our census of the local group, allowing us to
  locate ultra-faint dwarf satellites of the Milky Way, in effect
  solving the "missing satellites" problem.  LSST will likely reveal
  hundreds more of these satellites.

\medskip
\noindent {\bf Data Science Challenge 7: Dynamics of newly discovered Ultra-Faint dwarfs.}

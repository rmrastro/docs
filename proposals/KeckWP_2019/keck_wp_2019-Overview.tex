...
and multiple sampling formats; our first-light format is a
single-fiber mode. When configured in this mode, each Starbug carries
a single optical fiber with a 150 $\mu$m diameter core fed by
demagnifying fore-optics that yield a 0.9-arcsec diameter on-sky
aperture. When configured in multi-IFU mode, FOBOS would deploy a
...
monolithic-IFU mode would provide uniform, $\gtrsim$0.5 arcmin$^2$
field-of-view. Its flexible focal plane allows efficient observing
...
\comment{need to shorten the above}
